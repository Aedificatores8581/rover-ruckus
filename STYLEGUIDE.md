...
Flexibility exists, and breaking any conventions set is ok if the programmer is concious of his/her decision, and it doesn't
hinder readibility/writability

### BRACKETS

Use *Egyption* Brackets:
```java
if (bar == 30) {
...
```java
while (true) {if (shorty != 3) foo();} // This is just confusing.
```
...
Also with if-statements, try and keep the same style throughout the entirety of the if-else clause
...
. . . but in my opinion, that just looks like yucky poo poo B.S. So, try not chain single-liners
...
/* However, the only thing that isn't fine is making a lot of unneccesary space, that includes
...
int elephants = 1;
```
And use spaces not tabs, see spacing.
...
For anything that is a "attribute" use snake case to delinate that type.
